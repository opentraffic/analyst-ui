/* global it, expect */
import { getRouteTime, getSpeedFromDataTilesForSegmentId, getNextSegmentDelayFromDataTiles } from './route-time'
...
it('consolidates route attribute response into a keyed object', () => {
  ...
  describe('traffic segments parsing...', () => {
    ...
    test('has a segment_id', () => {
      expect(traceAttributes.edges.traffic_segments).toBe('251658259320')
    })
  })

  describe('speed returned from data tiles', () => {
    test('currently always should return 30', () => {
      expect(getSpeedFromDataTilesForSegmentId(251658259320)).toEqual(30)
    })
  })

  describe('next segment delay from data tiles', () => {
    test('currently always should return 0', () => {
      expect(getNextSegmentDelayFromDataTiles(251658259320, 231559154552)).toEqual(0)
    })
  })

  describe('route time for route', () => {
    test('given trace attributes', () => {
      const routeTime = getRouteTime(traceAttributes)
      expect(routeTime).toEqual(0)  /* need to change */
    })
  })
})
